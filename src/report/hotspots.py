...
def create_hotspot_table(hotspots):
    ...
    if not hotspots:
        ...
        content = [
            Spacer(1, 5*cm),
            Paragraph(
                "<i>No security hotspots found. This indicates good security practices in the codebase.</i>", 
                style_normal
            )
        ]
    ...
    for hotspot in sorted_hotspots:
        ...
        risk_badge = CircleBadge(hotspot.vulnerability_probability[0].upper(), radius=8, color=prob_color)
    ...
    for i, row in enumerate(table_data[1:], start=1):  # Skip header
        if (isinstance(row[1], str) and row[1] == "" and 
            ...
            table_style.append(("BACKGROUND", (0, i), (-1, i), colors.Color(1.0, 0.98, 0.98)))  # Very light red
...
def generate_security_hotspots_page(report, elements):
    ...
    if categories:
        ...
        for category in sorted_categories:
            ...
            category_title = f"{format_security_category_name(category)} ({len(category_hotspots)} hotspots)"
    ...
    if uncategorized:
        ...
        elements.append(Paragraph(f"Other Security Hotspots ({len(uncategorized)} hotspots)", category_style))
